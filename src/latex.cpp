...
std::string Latex::_find_katex_path()
{
	...
	return "./katex"; // Completely removed and simplified to look for the katex/ in the Latexcpp_Demo/ folder
				  ...
				  ImageFormat format) const
{
	if (filepath.find_first_of('/')) {
		std::string folders = "mkdir " + std::string(filepath.substr(0,filepath.find_last_of('/')));
		std::system(folders.c_str());
	}
